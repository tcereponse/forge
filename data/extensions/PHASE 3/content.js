...
async function poll() {
    try {
        ...
        if (serverPhase !== 3) {
            if (isActivated) {
                console.log("💤 Phase 3 : Mise en veille (Phase " + serverPhase + " détectée)");
                isActivated = false;
            }
            return;
        }
        ...
        if (!isActivated) {
            console.log("🛡️ Phase 3 : ACTIVATION (Pureté Clinique)");
            isActivated = true;
        }
        
        if (data.status === "prompt" && data.prompt && data.prompt !== lastPrompt) {
            lastPrompt = data.prompt;
            console.log("🛡️ Phase 3 : Injection du Protocole de Pureté...");
...
function injectPrompt(text) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            if (sendBtn && !sendBtn.disabled) {
                console.log("🚀 [P3] Bouton Send trouvé et cliqué !", sendBtn);
                sendBtn.click();
            } else {
                console.warn("⚠️ [P3] Bouton introuvable, fallback Enter agressif...");
    ...
    } catch (err) { console.error("❌ Erreur injection Phase 3:", err); }
}
    ...
    try {
        ...
        if (!isGenerating && content.length > 200 && content !== state.lastCapturedContent) {
            ...
            if (content.includes("Fichier :") || content.includes("FILE:") || content.includes("```") || content.includes("Copier") || content.includes("Télécharger") || /^\d+\.\s+[\w\.\-\/]+/m.test(content)) {
                ...
                console.log("📥 Capture P3 envoyée (" + content.length + " chars)");
            }
        }
    } catch (e) {}
}
