...
async function poll() {
    try {
        ...
        if (serverPhase !== 5) {
            if (isActivated) {
                console.log("💤 Phase 5 : Mise en veille (Phase " + serverPhase + " détectée)");
                isActivated = false;
            }
            return;
        }
        ...
        if (!isActivated) {
            console.log("🛠️ Phase 5 : ACTIVATION (Moteur de Refactoring)");
            isActivated = true;
        }
        
        if (data.status === "prompt" && data.prompt && data.prompt !== lastPrompt) {
            lastPrompt = data.prompt;
            console.log("💉 Injection du Patch de Refactoring...");
    ...
    try {
        ...
        setTimeout(() => {
            ...
            if (sendBtn && !sendBtn.disabled) {
                console.log("🚀 [P5] Bouton Send trouvé et cliqué !", sendBtn);
                sendBtn.click();
            } else {
                console.warn("⚠️ [P5] Bouton introuvable, fallback Enter agressif...");
        ...
        console.error("❌ Erreur injection P5:", err);
    }
}
    ...
    try {
        ...
        if (!isGenerating && content.length > 200 && content !== state.lastCapturedContent) {
            if (content.includes("Fichier :") || content.includes("FILE:") || content.includes("```") || content.includes("Copier") || content.includes("Télécharger") || /^\d+\.\s+[\w\.\-\/]+/m.test(content)) {
                ...
                console.log("📥 Capture P5 envoyée (" + content.length + " chars)");
            }
        }
    } catch (e) {}
}
