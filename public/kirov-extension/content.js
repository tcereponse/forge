...
window.addEventListener('unhandledrejection', event => {
    console.error('[KIROV3] Unhandled Promise Rejection Detected:', {
        reason: event.reason,
...
window.addEventListener('error', event => {
    console.error('[KIROV3] Global Error Detected:', {
...
const CONFIG = {
    ...
    MAX_PAYLOAD_SIZE: 5 * 1024 * 1024, // 5MB limite
...
class BridgeClient {
    ...
    async poll() {
        ...
        try {
            ...
            // Simulate the format the PromptEngine expects
            if (data && data.prompt) {
                const eventData = {
                    status: "prompt",
                    prompt: data.prompt,
                    project_id: data.projectId,
                    phase_num: 2 // Assuming phase 2 for generation
                };
                bus.emit('BRIDGE_SYNC', eventData);
            }
    ...
    async pollLogs() {
        try {
            const url = `${CONFIG.SERVER_URL}/v1/logs`;
            KirovLogger.info(`[POLL_LOGS] Fetching URL: ${url}`);
            const res = await fetch(url);
            KirovLogger.info(`[POLL_LOGS] Status: ${res.status}, Content-Type: ${res.headers.get("content-type")}`);
            if (!res.ok) return;
...
class PromptEngine {
    ...
    async handleSync(data) {
        ...
        if (data.status === "prompt" && data.prompt) {
            ...
            // Déduplication avec SHA-256 persistant pour éviter les boucles au rechargement
...
`;
        ...
        if (phase === 1) {
            ...
            text += `[SKILL: DIAGNOSE] Utilise la méthode scientifique (Hypothèse -> Vérification) pour justifyer tes choix techniques.\n`;
    ...
    async inject(rawPrompt, phase, projectId) {
        // We use the rawPrompt from React Forge directly, which already contains the 49 packs
...
class OutputScanner {
    ...
    startObserving() {
        ...
        this.observer = new MutationObserver(() => {
            ...
            // Debounce de 3000ms — laisse le temps à DeepSeek de finir de streamer
            this.timeout = setTimeout(() => this.parseLatestMessage(), 3000);
        });
        this.observer.observe(container, { childList: true, characterData: true, subtree: true });
    }

    isStreamingComplete(msgElement) {
        // Vérifie si le bouton "Stop" est visible (DeepSeek, ChatGPT, etc.)
        const stopSelectors = [
        ...
        for (const sel of stopSelectors) {
            ...
            const visible = Array.from(els).some(el => el.getBoundingClientRect().width > 0 || el.offsetParent !== null);
    ...
    async parseLatestMessage() {
        ...
        // ── GARDE MINIMALE : 45s après injection pour éviter de capturer le <think> de DeepSeek R1
        if (promptEngine.injectionTime) {
            const elapsed = Date.now() - promptEngine.injectionTime;
            if (elapsed < 45000) {
                KirovLogger.debug(`Capture bloquée (trop tôt: ${Math.round(elapsed/1000)}s / 45s min)`);
                return;
            }
        }

        // Force capture after 5 minutes even if we think it's still streaming
        const isTimeout = promptEngine.injectionTime && (Date.now() - promptEngine.injectionTime > 5 * 60 * 1000);
        
        if (!this.isStreamingComplete(lastMsg) && !isTimeout) return;
        if (isTimeout) {
            KirovLogger.warn("Force capture triggered due to 4 minute timeout.");
            promptEngine.injectionTime = null; // Reset
        }

        // Clone du message pour le nettoyer sans modifier le DOM visible
        const clone = lastMsg.cloneNode(true);
        // Position offscreen so it doesn't cause layout shift or flashes
        clone.style.position = 'absolute';
        clone.style.left = '-9999px';
        clone.style.top = '-9999px';
        clone.style.visibility = 'hidden';

        // Supprimer les boutons et en-têtes de l'interface qui génèrent "Copier", "Télécharger" ou des langues
        const uiElements = clone.querySelectorAll('button, svg, [role="button"], [class*="action"], [class*="copy"], [class*="download"], [class*="header"], [class*="toolbar"], [class*="button"]');
        uiElements.forEach(el => el.remove());

        // FIX SOUVERAIN : Préserver les sauts de ligne en appendant temporairement le clone au DOM actif
        ...
        // Condition de déclenchement (fix: regex correct + check DOM reliable)
        // - "Fichier :" ou "File :" → format texte brut DeepSeek
        // - présence de blocs <pre> ou <code> dans le clone → blocs de code rendus par DeepSeek
        // - "PRD" → Phase 1
        const hasCodeBlocks = lastMsg.querySelectorAll('pre, code').length > 0;
        const hasFichierKeyword = /Fichier\s*:|FILE\s*:|File\s*:/i.test(content);
        const hasPRD = content.includes("PRD");
        const hasJsonFormat = content.trim().startsWith('{') || content.includes('"files"');

        if (hasFichierKeyword || hasCodeBlocks || hasPRD || hasJsonFormat) {
            ...
            KirovLogger.info(`New valid code block captured. (codeBlocks=${hasCodeBlocks}, fichier=${hasFichierKeyword})`);
            bridge.sendCapture({ 
                content: content,
                validation: validationResult
            });
        } else {
            KirovLogger.warn(`Content not captured: no trigger found (len=${content.length}, preview="${content.substring(0, 80)}")`);
        }
    }
}
